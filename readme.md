...
|------|------|
| `投稿 [图片]` | 上传语录图片 |
| `语录 [tag]` | 随机获取语录（tag 加权） |
| `语录.验证 <code>` | 验证身份，获取 Web Token |
| `语录.重置` | 撤销当前 Token，重新验证 |
| `语录.删除群 <群号>` | 从访问列表移除群 |
| `语录.设权限 <qqId> <role>` | Owner 设置用户权限（user/admin） |
| `语录.用户列表` | Owner 查看所有用户 |
| `语录.删除用户 <qqId>` | Owner 删除用户 |
...
| 字段 | 说明 |
|------|------|
| `autoToken` | 开启自动获取 Token |
| `apiSecret` | Web 端调 Bot API 的共享密钥 |
| `ownerId` | Owner 的用户 ID（自动获得 owner 权限） |
