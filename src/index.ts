...
export const Config = Schema.intersect([
  ...
  Schema.object({
    ...
    recordLink: Schema.dict(Schema.object({
    ...
    ctx.command('投稿 [param]')
      .action(async ({session}, param) => {
        if (utils.detectControl(controlJson, session.guildId, "record") &&
          ...
          return await utils.addRecord(ctx, session.gid.replaceAll(':', '_'), imageSrc);
        }
      })
    ctx.command('语录 [tag:string]')
      .action(async ({session}, tag) => {
        if (utils.detectControl(controlJson, session.guildId, "record") &&
          ...
          const filepath = await utils.getRecord(cfg, session.gid.replaceAll(':', '_'), tag);
